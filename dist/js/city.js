(() => {
    ...
    function tabs() {
        ...
        function setTabsStatus(tabsBlock) {
            ...
            const tabsSpollersMedia = tabsBlock.getAttribute("data-tabs") ? tabsBlock.getAttribute("data-tabs") : null;
                ...
                tabsContent.forEach(((tabsContentItem, index) => {
                    if (tabsTitles[index].classList.contains("_tab-active")) {
                        if (tabsBlockAnimate) {
                            ...
                            if (tabsSpollersMedia && window.innerWidth <= tabsSpollersMedia) setTimeout((() => {
                                tabsTitles[index].scrollIntoView({
                                    behavior: "smooth",
                                    block: "center",
                                    inline: "nearest"
                                });
                            }), tabsBlockAnimate / 1.2);
    ...
    let isFirst = false;
    let updatedArgumentToScroll = null;
    function gotoBlock(targetBlock, noHeader = false, speed = 500, offsetTop = 0, behavior = "smooth") {
        let args = Array.from(arguments);
        args[4] = "instant";
        updatedArgumentToScroll = args;
        const targetBlockElement = document.querySelector(targetBlock);
        if (targetBlockElement) {
            let headerItem = "";
            let headerItemHeight = 0;
            if (noHeader) {
                headerItem = "header .header__container";
            ...
            if (typeof SmoothScroll !== "undefined") (new SmoothScroll).animateScroll(targetBlockElement, "", options); else scrollPageToElem(targetBlockElement, offsetTop, headerItemHeight);
        }
        if ((window.scrollY || window.pageYOffset) === 0 && !isFirst && window.innerWidth <= 479.98) setTimeout((() => {
            gotoBlock(...updatedArgumentToScroll);
            isFirst = true;
        }), 1500); else if ((window.scrollY || window.pageYOffset) === 0 && window.innerWidth >= 479.98) setTimeout((() => {
            gotoBlock(...updatedArgumentToScroll);
        }), 500);
    }
    function scrollPageToElem(targetBlockElement, offsetTop, headerItemHeight) {
        let targetBlockElementPosition = targetBlockElement.getBoundingClientRect().top + (window.scrollY || window.pageYOffset);
        ...
        function pageNavigationAction(e) {
            if (e.type === "click") {
                ...
                if (targetElement.closest("[data-goto]")) {
                    ...
                    gotoBlock(gotoLinkSelector, noHeader, gotoSpeed, offsetTop, "smooth");
                    e.preventDefault();
                }
    ...
    function sampleSlider() {
        const sliderBlock = document.querySelector('[data-slider="sample-slider"]');
        if (sliderBlock) {
            const slider = sliderBlock.querySelector(`[data-slider]`);
            const allSildes = sliderBlock.querySelectorAll(".swiper-slide");
            if (slider) new Swiper(slider, {
                centeredSlides: true,
                slidesPerView: 1,
                spaceBetween: -40,
                speed: 800,
                lazy: true,
                initialSlide: Math.floor(allSildes.length / 2) ? Math.floor(allSildes.length / 2) - 1 : Math.floor(allSildes.length / 2),
                breakpoints: {
                    320: {
                        spaceBetween: -35
                    },
    ...
    function showMoreHideGridElems() {
        ...
        function setMaxHeight(gridContainer, gridItems, row, btnShowHide, savedWidth) {
            if (savedWidth && window.innerWidth == savedWidth) return; else {
                ...
                const offset = Number(gridContainer.hasAttribute("data-offset") ? gridContainer.getAttribute("data-offset") : 0);
                let height = calculateHeight(gridContainer, gridItems, row);
        ...
        function extentGridHeight(gridContainer) {
            const offset = Number(gridContainer.hasAttribute("data-offset") ? gridContainer.getAttribute("data-offset") : 0);
            const scrollHeight = gridContainer.scrollHeight + offset;
            gridContainer.style.height = scrollHeight + "px";
        }
    }
